...
export function Wordmark({ className = '' }) {
  return (
    <span className={`font-display tracking-[0.18em] text-primary ${className}`} aria-label="No Time">
      NO<span className="text-foreground">&nbsp;</span>TIME
    </span>
  );
}
  ...
  return (
    ...
    >
      <nav className="mx-auto flex h-[68px] max-w-[90rem] items-center justify-between px-5 md:px-10" aria-label={t.nav.label}>
        <a href="#top" className="text-xl md:text-2xl" aria-label={t.nav.home}>
          <Wordmark />
        </a>
